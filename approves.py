...
#Coded By @im_piro, Contact for support
...
LOG_ID = int("-1002578051346") #Coded By @im_piro, Contact for support
#Coded By @im_piro, Contact for support
...
async def is_subscribed(filter, client, update):
    ...
    try:
        ...
        return False#Coded By @im_piro, Contact for support
...
async def start(bot, message):
    ...
    start_msg = (
        ...
        "**Completely Developed By @im_piro**\n"
...
@app.on_chat_join_request()#Coded By @im_piro, Contact for support
async def handle_join_request(client, request: ChatJoinRequest):
    ...
    else:#Coded By @im_piro, Contact for support
        default = await app.get_messages(welcome_channel_id, int(DEFAULT_WELCOME_MSG))
        copied_message = await default.copy(user_id)
...
channel_cache = TTLCache(maxsize=1000, ttl=3600)#Coded By @im_piro, Contact for support
...
#Coded By @im_piro, Contact for support
    ...
    try:
        if farewell_msg == DEFAULT_FAREWELL_MSG:
            ...
            await callback_query.message.reply(msg)#Coded By @im_piro, Contact for support
        else:
...
async def accept_all(bot, callback_query: CallbackQuery):
    await callback_query.answer("Coming Very Soon, Stay Updated...! ✅", show_alert=True)#Coded By @im_piro, Contact for support
#----------------------------------------------------------------------------------------------
...
async def remove_channel(bot, callback_query: CallbackQuery):
    ...
    if not channel:
        await callback_query.answer("This channel is not configured for you.", show_alert=True)
        return#Coded By @im_piro, Contact for support
      ...
      #Coded By @im_piro, Contact for support      
            if chat.type == ChatType.SUPERGROUP or chat.type == ChatType.GROUP:
                ...
                if not channels.find_one({"chat_id": chat.id}):
                    ...
                    try:#Coded By @im_piro, Contact for support
                        await app.send_message(
                            chat_id=LOG_ID,
                            text=f"New Channel\n\n"
                                 f"Chat: {chat_title}\n"
...
if __name__ == "__main__":
    logger.info("Bot is started.. Coded By @im_piro, Contact for support.") 
    app.run()  
    logger.info("Bot has stopped. Coded By @im_piro, Contact for support") 